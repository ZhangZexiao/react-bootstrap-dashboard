...
function DashboardDropdownMenu(props) {
    ...
    return (
        <Dropdown id="" className="align-self-center" pullRight>
            <Dropdown.Toggle noCaret bsStyle="transparent">...</Dropdown.Toggle>
            <Dropdown.Menu>
                <MenuItem eventKey="1">{viewSettings}</MenuItem>
                <MenuItem eventKey="2">{exportPdf}</MenuItem>
                <MenuItem eventKey="3" disabled>{customizeDashboard}</MenuItem>
            </Dropdown.Menu>
        </Dropdown>
    )
}
...
class DashboardHead extends Component {
    ...
    render() {
        ...
        const separator = " | "
        const nextScanningDateDescription = "Next recurrence scan will take place on "
        return (
